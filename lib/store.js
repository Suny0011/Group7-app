...
export function addPending(actionObject) {
...
export function removePending(id) {
  write(PENDING, getPending().filter((p) => p.id !== id));
}

export function updatePending(id, status) {
  const all = getPending();
  const item = all.find((p) => p.id === id);
  if (item) { item.status = status; write(PENDING, all); }
}
